...
powershell -Command "& {$PSDefaultParameterValues['Out-File:Encoding'] = 'utf8'; (get-content %SCRIPT_DIR%..\Richter.jucer).replace(\"..\JUCE\modules\",\"JUCE\modules\") | out-file -filepath %SCRIPT_DIR%..\Richter.jucer}"

ECHO "=== Updating include paths ==="
powershell -Command "& {$PSDefaultParameterValues['Out-File:Encoding'] = 'utf8'; (get-content %SCRIPT_DIR%..\Richter.jucer).replace(\"%%USERPROFILE%%\WEA\WECore\WECore\",\"..\..\WECore\WECore\") | out-file -filepath %SCRIPT_DIR%..\Richter.jucer}"

ECHO "=== Generating project ==="
%SCRIPT_DIR%..\JUCE\Projucer --resave %SCRIPT_DIR%..\Richter.jucer

ECHO "=== Starting build ==="
cd %SCRIPT_DIR%..\Builds\VisualStudio2019
msbuild -version
msbuild Richter.sln /p:Configuration=Debug /p:Platform=x64
